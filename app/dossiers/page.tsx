...
export async function generateMetadata({
  ...
  const frBase = `${SITE_URL}/dossiers`
  const enBase = `${SITE_URL}/en/dossiers`
  const base = locale === 'en' ? enBase : frBase
  const canonicalUrl = page === 1 ? base : `${base}?page=${page}`
  const sectionTitle = t.sections.dossiers.title
  const title = page === 1 ? sectionTitle : `${sectionTitle}, page ${page}`
  const description = t.sections.dossiers.description
  return {
    title,
    description,
    alternates: {
