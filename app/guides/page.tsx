...
export async function generateMetadata({
  ...
  const frBase = `${SITE_URL}/guides`
  const enBase = `${SITE_URL}/en/guides`
  const base = locale === 'en' ? enBase : frBase
  const canonicalUrl = page === 1 ? base : `${base}?page=${page}`
  const sectionTitle = t.sections.guides.title
  const title = page === 1 ? sectionTitle : `${sectionTitle}, page ${page}`
  const description = t.sections.guides.description
  return {
...
export default async function GuidesPage({
  ...
  const locale = (headers().get('x-locale') ?? 'fr') as Locale
  const t = getT(locale)
  const s = t.sections.guides

  const allArticles = await enrichArticlesWithCovers(getArticlesByCategory('guides', locale))
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 md:px-8">
      <div className="mb-12 border-b border-line pb-8">
        <div className="flex items-center gap-4">
          <span className="block h-10 w-1.5 bg-sky-500" />
          <div>
            <p className="font-display text-xs uppercase tracking-widest text-sky-400">{t.sections.label}</p>
